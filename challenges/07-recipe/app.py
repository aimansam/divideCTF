...
RECIPES = {
    1:  {"title": "Spicy Garlic Noodles", "desc": "Quick wok-tossed noodles with garlic and chili.",
         "content": "Ingredients: noodles, garlic, chili flakes, soy sauce, scallions.\nSteps: Boil noodles. Sauté garlic + chili. Toss with soy sauce. Top scallions."},
    ...
    30: {"title": "???", "desc": "Locked.", "content": ""},  # placeholder only
}

@app.get("/")
def index():
    # Uses local images: /static/img/1.jpg ... /static/img/30.jpg
    days = []
    for d in range(1, 31):
        days.append({
            "day": d,
...
@app.get("/api/recipe/<int:day>")
def api_recipe(day: int):
    if day < 1 or day > 30:
        abort(404)
    ...
    # Day 30 is server-side only (flag is NOT in HTML/source)
    flag = os.environ.get("FLAG", "divide{cookmesometh1ngpleas3_182heb4j5n2}")
    return jsonify({
        "day": 30,
        "title": "Day 30: Rendang Royale",
        "desc": "The final recipe reveals the secret ingredient.",
        "content": f"Secret ingredient: {flag}"
    })
